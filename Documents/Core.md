...
该流程应当由客户端的开发者在**认证服务器的官方网站中**或**线下**向认证服务器完成注册。
...
该流程应当由资源服务器的开发者在**认证服务器的官方网站中**或**线下**向认证服务器完成注册。
...
| `scope_names`  | (字符串数组) 资源服务器提供的数据范围名称列表，对于其中每一个元素 `scope_name`，都表示资源服务器提供的数据范围名称 |

> [!TIP]
> 注意: `service_name` 和 `scope_name` 必须遵循下划线命名法，禁止包含空格，且不应包含大写字母，不推荐使用拼音。
