...
3. **认证服务器**将**用户**重定向到**客户端**的 `after_auth_redirect_url`，并附上以下内容作为查询:
   ...
   | 内容   | 解释                                                        |
   | ------ | ----------------------------------------------------------- |
   | `code` | (由**认证服务器**加密，以字符串形式传输) 资源访问令牌兑换码 |
...
6. **认证服务器**对于来自**客户端**的请求返回如下内容:
   ...
   | 内容           | 解释                                    |
   | -------------- | --------------------------------------- |
   | `access_token` | (由**认证服务器**加密) 资源访问令牌     |
...
1. **客户端**向**资源服务器**的 `resource_url` 发送 `POST` 请求，并附上以下内容作为 Body 部分:
   ...
   | -------------- | ----------------------------------------------------------------------------------------------------------------- |
   | `client_id`    | 客户端唯一标识符                                                                                                  |
   | `access_token` | (**客户端**与**认证服务器**之间，由**客户端**加密) 资源访问令牌                                                   |
   | `scope`        | (**客户端**与**认证服务器**之间，由**客户端**加密) 客户端将要尝试访问的数据范围，(对于原文而言)，多个，由空格隔开 |
   | `tdt`          | (**客户端**与**认证服务器**之间，由**客户端**加密) TDT 值                                                         |
...
2. **资源服务器**向**认证服务器**的 `authentication_url` 发送 `POST` 请求，并附上以下内容作为 Body 部分:
   ...
   | `client_tdtt`         | 客户端 `tdt`，原样发送                                            |
   | `tdt`                 | (**资源服务器**与**认证服务器**之间，由**资源服务器**加密) TDT 值 |
...
4. **认证服务器**对于来自**资源服务器**的请求返回如下内容:
   ...
   | 内容                   | 解释                                                                                                      |
   | ---------------------- | --------------------------------------------------------------------------------------------------------- |
   | `scope`                | (**认证服务器**与**资源服务器**之间，由**认证服务器**加密) 认证服务器允许客户端访问的数据范围             |
   | `scope_to_client`      | (**认证服务器**与**客户端**之间，由**认证服务器**加密) 认证服务器允许客户端访问的数据范围                 |
   | `account_id`           | (**认证服务器**与**资源服务器**之间，由**认证服务器**加密) 客户端 `access_token` 所绑定的用户的唯一标识符 |
   | `account_id_to_client` | (**认证服务器**与**客户端**之间，由**认证服务器**加密) 认证服务器允许客户端访问的数据范围                 |
...
1. **客户端**向**认证服务器**的 `update_url` 发送 `POST` 请求，并附上以下内容作为 Body 部分:
   ...
   | -------------- | ----------------------------------- |
   | `client_id`    | 客户端唯一标识符                    |
   | `access_token` | (由**客户端**加密) 旧的资源访问令牌 |
   | `tdt`          | (由**客户端**加密) TDT 值           |
...
2. **认证服务器**对于来自**客户端**的请求返回如下内容:
   ...
   | 内容           | 解释                                    |
   | -------------- | --------------------------------------- |
   | `access_token` | (由**认证服务器**加密) 新的资源访问令牌 |
   | `expire_time`  | 到期时间，为 `yyyy-MM-dd HH-mm-ss` 格式 |
